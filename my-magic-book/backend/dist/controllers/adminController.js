...
const generateColoringPreview = async (req, res) => {
    try {
        ...
        const derived = (0, sceneTemplates_2.resolveColoringScenes)((0, sceneTemplates_2.getSceneTemplate)(themeId));
        ...
        const coverScene = req.body?.coloringCoverScene || theme.coloringCoverScene
            || derived?.cover || `exploring ${theme.label}`;
        const backScene = req.body?.coloringBackCoverScene || theme.coloringBackCoverScene
        ...
        const cover = await (0, ImageGenerator_1.generateIllustration)((0, sceneTemplates_1.buildColoringCoverPrompt)(coverScene, childName, childGender), referencePhoto, { storyId: folder, pageNumber: 0 });
        // 2) 16 line-art pages
        const generatedImages = [];
        for (let i = 0; coverOnly ? false : i < sceneTemplates_1.COLORING_PAGES; i++) {
            const scene = cleanScenes[i] || cleanScenes[cleanScenes.length - 1];
            const img = await (0, ImageGenerator_1.generateIllustration)((0, sceneTemplates_1.buildScenePrompt)('page', scene, childName, childGender, { coloring: true }), referencePhoto, { storyId: folder, pageNumber: i + 1 });
            generatedImages.push(img.objectPath);
        }
        // 3) colored back cover
        const back = coverOnly ? null : await (0, ImageGenerator_1.generateIllustration)((0, sceneTemplates_1.buildColoringBackCoverPrompt)(backScene, childName, childGender), referencePhoto, { storyId: folder, pageNumber: 98 });
        // Stored beside the story's artwork, not on top of it.
        theme.coloringCover = cover.objectPath;
        if (generatedImages.length)
            theme.coloringImages = generatedImages;
        if (back)
            theme.coloringBackCover = back.objectPath;
        settings.markModified('themes');
        await settings.save();
...
const trackVisit = async (req, res) => {
    try {
        ...
        const returning = (await Visit_1.default.countDocuments({ visitorId, day: { $ne: day } })) > 0;
...
const listVisits = async (req, res) => {
    try {
        ...
        const week = await Visit_1.default.aggregate([
...
const listCustomers = async (_req, res) => {
    try {
        const users = await User_1.default.find({}).select('name email role phone location createdAt lastLoginAt loginCount lastSeenAt loginHistory').sort({ createdAt: -1 }).lean();
        ...
        const visitRows = await Visit_1.default.find({ userId: { $ne: null } })
