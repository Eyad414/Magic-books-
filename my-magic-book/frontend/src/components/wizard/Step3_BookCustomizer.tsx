...
export default function Step3_BookCustomizer({ onNext, onPrev }: Props) {
  const { progress, setBookCustomization } = useStoryProgress();
          ...
          <div className="flex gap-2 w-full">
            {packages.map((pkg) => (
              <button
                ...
                className={`relative flex-1 flex flex-col items-center justify-center p-2 h-28 rounded-2xl border-2 transition-all text-center group ${
